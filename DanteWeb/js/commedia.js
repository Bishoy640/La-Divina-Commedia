        ...
        for (let terzine of canto["children"]) {
          for (let riga of terzine["children"]) {
            riga["text"] = riga["text"]
              ...
              .replace("\u00ec", "i");
            if (
              riga["text"].toUpperCase().includes(document.getElementById("myInput").value.toUpperCase())
            ) {
              answer +=
                "Cantica: " +
                cantica["name"] +
          ...
          if (canto["name"] == CantoNum) {
            ...
            for (let terzine of canto["children"]) {
              ...
              for (let riga of terzine["children"]) {
                ...
                answer += riga["text"] + "<p style='color: grey; display:inline; margin-left: .5em'>"+ counterRiga +"</p>";
                answer += "<br>";
